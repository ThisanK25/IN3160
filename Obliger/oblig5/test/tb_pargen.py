...
async def stimuli_generator(dut):
    indata1_pattern = [random.randint(0, 255) for _ in range(20)]
    indata2_pattern = [i + 1 for i in range(20)]
...
async def compare(dut):
    ...
    while True:
        await RisingEdge(dut.mclk)
        await ReadOnly()
        assert dut.toggle_parity.value == parity(dut.indata1.value), f"toggle_parity = {dut.toggle_parity.value}"
        assert dut.xor_parity.value == parity(dut.indata2.value) % 2, f"xor_parity = {dut.xor_parity.value}"
