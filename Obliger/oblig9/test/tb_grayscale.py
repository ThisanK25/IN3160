...
SCALING = 3      # 1 renders full image size.
...
async def gray_check(dut):
    while True:
        ...
        await RisingEdge(dut.clk) # Comment out to run on non-pipelined architecture
        await ReadOnly()
        if dut.valid_pipelined == True:
            sum = CheckR + CheckG + CheckB
            assert sum == int(dut.r_Y.value)
        await RisingEdge(dut.clk)
        await ReadOnly()
...
async def valid_check(dut):
    while True:
        ...
        await RisingEdge(dut.clk)
        await ReadOnly()
        tmp = dut.valid_pipelined.value
        assert data_valid == tmp
        await RisingEdge(dut.clk)
        await ReadOnly()
...
async def main_test(dut):
    ...
    img = Image.open('images/cheems.png')
    width = int(img.width/SCALING)
    height = int(img.height/SCALING)
